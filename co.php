...
<h1 align=center style="font-family:cambria;font-size:20">STUDIES WITH ENJOYEMENT!!</h1>	
<?php
$conn=mysql_connect("localhost","root","student");
mysql_select_db("web",$conn);
$sql="select * from coactivity";
$resultSet=mysql_query($sql,$conn);
echo "<table  border='3' cellspacing='5' cellpadding='5'  height='35%' width='65%' style='margin-top:-5spx;margin-left:50px'>";
echo "<tr style='font-family:cambria' align=center><td>Sr NO.</td><td>ACTIVITY NAME</td><td>FACULTY NAME</td><td>PARTICIPATED IN</td><td>TEAM NAME</td></tr>";
$i=1;
while($row=mysql_fetch_array($resultSet))
{
echo "<tr align='center'><td>$i</td><td style='font-family:cambria'>$row[activity_name]</td><td>$row[faculty_name]</td><td>$row[participated_in]</td><td>$row[team_name]</td></tr>";
$i++;
}
echo "</table>"
?>
